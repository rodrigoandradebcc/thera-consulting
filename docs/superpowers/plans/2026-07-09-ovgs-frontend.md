        ...
        {items.map((line) => (
          <TableRow key={line.itemId}>
            ...
            <TableCell className="tabular text-right">
              {money(((Number(line.unitPrice) * 100 * line.quantity) / 100).toFixed(2))}
            </TableCell>
          </TableRow>
        ))}
        <TableRow>
...
A página lista os clientes em tabela; clicar em uma linha expande `CustomerTransportTypes` abaixo dela. Botão "Desativar" com o mesmo dialog de confirmação do `ToggleActive`.
